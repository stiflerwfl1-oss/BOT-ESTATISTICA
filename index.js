...
function buildLegacyMembers(period) {
  ...
  keys.forEach((key) => {
    ...
    Object.entries(bucket).forEach(([userId, stats]) => {
      const row = ensureLegacyMemberRow(byUser, userId);
      const voiceMs = Number(stats.voice || 0);
      const activeVoiceMsRaw = Number(stats.activeVoice);
      const activeVoiceMs = Number.isFinite(activeVoiceMsRaw) && activeVoiceMsRaw > 0 ? activeVoiceMsRaw : voiceMs;
      const gameMs = Number(stats.game || 0);
      row.online_minutes += Math.round(Number(stats.online || 0) / 60_000);
      row.voice_minutes += Math.round(voiceMs / 60_000);
      row.active_voice_minutes += Math.round(activeVoiceMs / 60_000);
      row.game_minutes += Math.round(gameMs / 60_000);
      row.messages += Number(stats.messages || 0);
    });
  });

  addLiveDurationsToLegacyMembers(byUser, period);

  const rows = Object.values(byUser).map((row) => {
    const user = db.users[row.user_id] || {};
  ...
  if (rows.length > 0) {
    rows.sort((a, b) => b.active_voice_minutes - a.active_voice_minutes);
    return rows;
  }

  const fallbackRows = getLiveStats()
    .map((user) => {
      const names = splitTag(user.tag);
      return {
        user_id: user.id,
        ...
        active_voice_minutes: Math.round(
          Number((user.totalActiveVoiceTime || user.totalVoiceTime || 0)) / 60_000
        ),
    ...
    .sort((a, b) => b.active_voice_minutes - a.active_voice_minutes);
